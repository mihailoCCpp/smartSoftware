    ...
    public class SmartSoftwareDocumentVM : IViewModel<SmartSoftwareDocument>
    {
        ...
        private void refresujOblastiITipove(SmartSoftwareDocument document)
        {
            ...
            for (int i = 0; i < nizOblasti.Length; i++)
            {
                ...
                OblastiOpreme o = new OblastiOpreme(this.smartSoftwareDocument.Root) { Name = nizOblasti[i].naziv_oblasti_opreme, IdOblastiOpreme = nizOblasti[i].id_oblasti_opreme, Picture = nizOblasti[i].picture };
